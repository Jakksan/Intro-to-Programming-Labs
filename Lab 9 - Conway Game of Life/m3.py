...
survivors = {} # [[(x, y), age], [(x2, y2), age]
def organismState(x, y, pos_count):
    ...
    # covers rule 2
    elif (pos_count == 3 or pos_count == 2) and matrix[x][y] >= 1:
        pass
        survivors.update({(x,y): age})
...
for frame in range(framecount):
    ...
    for each in range(int((width/square_size)/2)+1):
        if width_squares%2 == 1:
            each = each + 0.5
        penup()
        setpos(((each)*square_size)-(0.5*dot_size), ((height/2)-(0.5*dot_size))+dot_size)
        pendown()
        setpos(((each)*square_size)-(0.5*dot_size), ((-height/2)-(0.5*dot_size))+dot_size)
        penup()
        setpos(-((each)*square_size)-(0.5*dot_size), ((height/2)-(0.5*dot_size))+dot_size)
    ...
    for each in range(int((height/square_size)/2)+1):
        if height_squares%2 == 1:
            each = each + 0.5
        penup()
        setpos((width/2)-(0.5*dot_size), ((each)*square_size)-(0.5*dot_size)+dot_size)
        pendown()
        setpos(-(width/2)-(0.5*dot_size), ((each)*square_size)-(0.5*dot_size)+dot_size)
        penup()
        setpos((width/2)-(0.5*dot_size), -((each)*square_size)-(0.5*dot_size)+dot_size)
    ...
    for i in range(y_len):
        ...
        for j in range(x_len):
            ...
            organismState(i, j, pos_count)
            age = critterLives(i,j)
            print(age)
